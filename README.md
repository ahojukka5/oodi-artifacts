...
For pull requests, it builds the Linux x86_64/glibc artifact and proves the
strict raw `GeomFill_Gordon` fixture together with the public
`gordon_surface` rectangle fixture. It uploads the deterministic archive,
provenance record, and build logs as workflow artifacts without publishing a
release.

After the workflow change reaches `main`, the same qualification publishes a
release named from the Julia artifact tree hash. Existing tags are immutable:
a rerun succeeds only when both the archive bytes and provenance are identical.
Every release records the wrapper source commit, qualification commit, platform,
